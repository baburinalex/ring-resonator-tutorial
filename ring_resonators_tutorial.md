...
## Как спроектировать резонатор: пошаговый чеклист

Этот раздел собирает всё вместе — от спецификации до готового устройства. Думай о нём как о to-do листе (галочки можно отмечать прямо на GitHub).

### Фаза 0. Спецификация и ограничения

- [ ] Записать, что нужно: рабочая длина волны $\lambda$, целевой FSR, целевой $Q$ (или полоса пропускания), нужен ли высокий ER (для фильтра) или нет.
- [ ] Зафиксировать платформу и её константы: $n_{\text{eff}}$, $n_g$, $\alpha_{\text{prop}}$, $\kappa_0$, $\gamma$ (у нас — SOI strip 220×500 нм, 1550 нм).
- [ ] Узнать fab-ограничения: минимальный зазор (≈ 50–100 нм) и минимальный радиус по изгибным потерям (≈ 5–8 мкм для SOI strip).

### Фаза 1. Геометрия из спецификации

- [ ] Радиус из FSR: $R = \lambda^2 / (2\pi\, n_g\, \text{FSR})$.
- [ ] Проверка изгиба: $R$ не меньше минимального? Если меньше — FSR слишком широкий для этой платформы; уменьши FSR или смени платформу (rib / SiN).

### Фаза 2. Проверка добротности

- [ ] Посчитать потолок: $Q_{\text{intrinsic}}(R) = 2\pi n_g / (\lambda\, \alpha)$.
- [ ] Условие $Q_{\text{target}} < Q_{\text{intrinsic}}$ выполнено? Если нет — **дизайн невозможен**: собственные потери кольца не дадут такую добротность. Увеличь $R$ (меньше изгиб) или снизь потери / смени платформу.

### Фаза 3. Связь и зазор

- [ ] Найти нужную $Q_{\text{coupling}}$ из $\frac{1}{Q_{\text{loaded}}} = \frac{1}{Q_{\text{int}}} + \frac{1}{Q_{\text{coup}}}$, затем перейти к $\kappa$.
- [ ] Перевести $\kappa$ в зазор: $g = -\frac{1}{\gamma}\ln(\kappa/\kappa_0)$.
- [ ] Зазор $g$ не меньше fab-предела? Если меньше — увеличь $\kappa_0$ (длиннее область связи) или прими меньший $Q$.
- [ ] Нужен высокий ER? Тогда целься в критическую связь: $\kappa^2 \approx 1 - e^{-\alpha L}$.

### Фаза 4. Проверка симуляцией

- [ ] Подставить $R$ и $g$ в `transmission()`, построить спектр.
- [ ] Сверить FSR, $Q$, ER с целями. Не сходится — вернись к нужной фазе и поправь.
- [ ] Всю обратную задачу разом делает `design_for_FSR_Q(FSR_target, Q_target)`.

### Фаза 5. Перед отправкой на изготовление

- [ ] $g \ge$ fab-предела и $R \ge$ изгибного предела.
- [ ] FSR влезает в окно измерения (диапазон лазера / анализатора спектра).
- [ ] Есть запас по критической связи, если нужен высокий ER.

### Фаза 6. После изготовления — характеризация

- [ ] Снять спектр и прогнать `spectrum_fit.py`.
- [ ] Сравнить восстановленные FSR / $Q$ / ER / $\alpha$ с расчётными.
- [ ] Помнить про вырождение $t \leftrightarrow A$: режим связи по одному модулю $|T|$ не определить — нужна доп. информация.

## Бонус: что происходит при росте потерь (реальный случай для КНИ)

![Рост потерь](images/fig6_loss_study.png)

Реалистичные потери распространения для SOI strip (КНИ) на 1550 нм: примерно **0.3–1 дБ/см** для оптимизированного процесса, **1–3 дБ/см** для типичного foundry, и до **3–10 дБ/см** при шероховатых стенках. Посмотрим, что происходит с одним и тем же кольцом (фиксированные $R$ и зазор), когда потери растут.

Рост потерь делает **две разные вещи**:

1. **Ширина растёт, Q падает — монотонно.** Чем больше потери, тем сильнее свет затухает за оборот, тем меньше добротность $Q$, тем шире провал (средняя панель).

2. **Глубина (ER) ведёт себя немонотонно.** Это неочевидно: при фиксированной связи рост потерь сначала *углубляет* провал, доводит его до максимума в точке **критической связи** ($A = t$, здесь ≈ 2.5 дБ/см), а затем делает его мельче. Слева хорошо видно: кривая критической связи проваливается почти до нуля, а кривые с меньшими и бо́льшими потерями — мельче.

Почему так: пока потери малы, кольцо overcoupled ($t < A$); с ростом потерь $A$ уменьшается и в какой-то момент сравнивается с $t$ — это критическая связь и максимальное гашение; дальше $A < t$, кольцо становится undercoupled, и провал снова мельчает.

```python
# код этой картинки — в ring_tutorial.py, функция fig6_loss_study()
from ring_tutorial import Q_loaded, extinction_ratio_dB, amplitude_loss
# при фиксированном gap меняем alpha и смотрим на Q (монотонно) и ER (с пиком)
```

**Практический вывод:** если на измеренном спектре провал неглубокий, это само по себе НЕ говорит, мало потерь или много — устройство может быть и overcoupled, и undercoupled. Различить помогает только дополнительная информация (см. инструмент характеризации спектра).
